...
class AsyncModelsProxy:
    ...
    async def _setup_and_run(self, loop_func, model: str, contents, config: types.GenerateContentConfig | None = None, **kwargs):
        manager = MCPConnectionManager(
            server_config=self._wrapper.mcp_servers,
            excluded_tools=self._wrapper.excluded_tools
        )
        ...
        try:
            ...
            if loop_func == generate_content_loop:
                await manager.close()
...
class MCPClientWrapper:
    ...
    """
    def __init__(self, base_client: genai.Client, mcp_servers: dict, excluded_tools: list = None):
